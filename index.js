  ...
  heroTl
    .from('.hero-tag',    { opacity: 0, y: 24, duration: .6, ease: 'power3.out' })
    .from('.hero-title',  { opacity: 0, y: 32, duration: .7, ease: 'power3.out' }, '-=.35')
    .from('.hero-desc',   { opacity: 0, y: 24, duration: .6, ease: 'power3.out' }, '-=.45')
    .from('.hero-ctas',   { opacity: 0, y: 20, duration: .5, ease: 'power3.out' }, '-=.4')
    .from('.trust-badges',{ opacity: 0, y: 16, duration: .5, ease: 'power3.out' }, '-=.35')
    .from('.sim-card',    { opacity: 0, x: 40, duration: .8, ease: 'power3.out' }, '-=.6');
  ...
  const statValues = [
    { el: document.querySelectorAll('.stats-value')[0], end: 1200, suffix: '+', prefix: '' },
    { el: document.querySelectorAll('.stats-value')[2], end: 24,   suffix: 'h', prefix: '' },
  ];
  ...
  document.querySelectorAll('.benefit-card').forEach(card => {
    card.addEventListener('mousemove', e => {
      ...
      gsap.to(card, { rotateY: x * 6, rotateX: -y * 6, duration: .3, ease: 'power1.out', transformPerspective: 800 });
    });
